...
TicTacToe::TicTacToe() : GridGame(3){
...
bool TicTacToe::placeMark(int row, int col, int p) {	//A virtual function for placing mark.
	if(row<0 || col<0 || row>= myGrid.arrSize || col>=myGrid.arrSize)
	{
		cout << "Please enter a valid number between 0 and " << myGrid.arrSize-1 << endl;
		return false;
	}
...
void TicTacToe::placeMark(int player) { //A virtual function for placing mark.
	smartMove();
}

int TicTacToe::getStatus() const{	//A  function for checking the game status.
	return gameStatus;
};

void TicTacToe::smartMove() {
